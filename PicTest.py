...
class GamePage(tk.Frame):
    def __init__(self, parent, controller, word_list=None):
        ...
        self.bg_photo = tk.PhotoImage(file="./Images/Background.png")
        self.bg_id = self.canvas.create_image(200, 200, image=self.bg_photo)
        ...
        self.word_label = ttk.Label(info_frame, text="", font=("Arial", 20))
        self.word_label.pack(pady=5)

        self.remaining_label = ttk.Label(info_frame, text="Guessed letters:", font=("Arial", 14))
        self.remaining_label.pack(pady=10)

        # Frame for input and guess button
        ...
        self.guess_entry = ttk.Entry(input_frame, width=10, font=("Arial", 18),
                                     validate="key", validatecommand=validate_command)
        self.guess_entry.pack(side="left", padx=10)

        self.guess_button = ttk.Button(input_frame, text="Guess", command=self.display_guess)
        self.guess_button.pack(side="right")

        # Frame for guessed letters display
        guess_frame = tk.Frame(self)
        guess_frame.pack(pady=10, fill="x")

        self.guess_info_display = ttk.Label(guess_frame, text="Guessed letters:", font=("Arial", 14))
        self.guess_info_display.pack(anchor="w", padx=10)

        self.guess_display = ttk.Label(guess_frame, text="", font=("Arial", 14))
        self.guess_display.pack(anchor="w", padx=10)
    ...
    def update_image(self):
        ...
        if self.wrong_guesses <= 6:
            filename = f"./Images/TikiMan1HB{self.wrong_guesses}.png"
            try:
                self.overlay_photo = tk.PhotoImage(file=filename)
                if self.overlay_id:
                    self.canvas.delete(self.overlay_id)
                self.overlay_id = self.canvas.create_image(200, 200, image=self.overlay_photo)
